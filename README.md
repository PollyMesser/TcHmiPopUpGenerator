...
- **Baukastenprinzip** – Popups aus wiederverwendbaren Bausteinen zusammensetzen, inklusive Statusanzeigen, Enums, Ladebalken, Trendkurven (Plot) und bedingt sichtbaren/aktiven Buttons.
...
> **Hinweis zum Plot-Baustein:** Der Verlauf-Plot ist bewusst **modus-unabhängig**. Er bringt seine eigene `TcHmi.Symbol`-Anbindung mit und abonniert seine Signale immer direkt per ADS – unabhängig davon, wie die übrigen Bausteine ihre Daten lesen. In allen drei Modi wird derselbe, in sich geschlossene Plot-Code erzeugt.
...
| **Eingabefeld** | Zahl- oder Texteingabe mit Senden-Button (Farbe wählbar), schreibt bei Klick oder Enter. Feldinhalt **rechtsbündig**, optionale **Einheit** als Zusatz neben dem Feld. |
...
1. Unter **Allgemein** den Ausgabe-Modus wählen, Funktionsname/UID, ggf. Auslöser-Suffix (UserControl-Modus), Titel(-Quelle), Breite und Spaltenzahl festlegen.
...
- Eine Zeile fasst maximal vier Elemente (nur read/bool/check/input, keine Enum-/Status-/Plot-Bausteine), ein Button-Baustein maximal zwei Buttons.

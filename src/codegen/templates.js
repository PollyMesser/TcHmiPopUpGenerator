...
// ── Innerer Rumpf (Embed-Modus): rendert in einen Zielcontainer statt als Overlay ──
// Bringt dieselben Helfer wie innerSymbol mit (loc/subscribe/writeSymbol/pulseSymbol/
// toggleSymbol/getPalette/teardowns), damit derselbe bodyContent unveraendert laeuft.
// Kein Overlay, kein Drag, kein Header-Close. Aufbau/Abbau idempotent ueber
// container.__acEmbedDestroy (mehrfaches onAttached ist damit unkritisch).
function innerEmbed(bodyContent) {
  return `                var container = resolveTarget(target);
